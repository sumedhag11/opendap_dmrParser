...
This tool has been tested against both THREDDS and Hyrax servers. 'describe' (this submission) is the first piece. The planned next piece is to implement 'query' that will pull actual data from '.ascii' files

# Testing
No automated tests are included in this submission. I would add tests to check parser against different datasets using CppUnit, in particular writing regression tests for the two cross-server encoding differences found during development, Hyrax's <Value>text</Value> vs. THREDDS's <Value value="text"/>.
